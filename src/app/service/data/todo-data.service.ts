...
export class TodoDataService {
  ...
  retrieveAllTodos(name: any): Observable<Todo[]> {
    return this.http.get<Todo[]>(`${API_URL}/jpa/users/${name}/todos`);
  }

  deleteTodo(name: any, id: number): Observable<Todo> {
    return this.http.delete<Todo>(`${API_URL}/jpa/users/${name}/todos/${id}`);
  }

  getTodo(name: any, id: number | undefined): Observable<Todo> {
    return this.http.get<Todo>(`${API_URL}/jpa/users/${name}/todos/${id}`);
  }

  updateTodo(name: any, id: number | undefined, todo: Todo) {
    return this.http.put(`${API_URL}/jpa/users/${name}/todos/${id}`, todo);
  }

  createTodo(name: any, id: number | undefined, todo: Todo): Observable<Todo> {
    return this.http.post<Todo>(`${API_URL}/jpa/users/${name}/todos`, todo);
  }
